...
```
```{r setuphidden, eval = TRUE, echo = FALSE, include=FALSE}
library(secrlinear)
library(rgdal)         # to read shapefiles
...
```{r plotglyme, eval = FALSE, fig.width = 7, fig.height=3}
par(mar = c(1,1,1,1))
plot(glymemask)
...
'.shp' extension. The function `readOGR` from **rgdal** (Bivand et al. 2005) is used internally to create a SpatialLinesDataFrame from the shapefile. 
...
Bivand, R., Keitt, T and Rowlingson, B. (2016). rgdal: Bindings
  for the Geospatial Data Abstraction Library. R package version 1.1-9.
